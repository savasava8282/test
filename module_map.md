...
| `src/weather_alert_bot/daily_summary.py` | Pure/deterministic daily summary builder и user-facing risk-section composition | updated | Сохраняет существующий weather/Kp formatter; добавляет concise risk block с явным immutable mapping категорий к warning settings и отдельное climate-unavailable note |
| `src/weather_alert_bot/today_handler.py` | Однократная обработка новой приватной `/today` владельца | updated | Read-only settings storage, writable separate climate cache, Weather/NOAA single-fetch reuse, all-eight assessment, per-category filtering и six-category climate fallback; подтверждён controlled real Telegram cache-hit тестом 14.08.2026 |
| `src/weather_alert_bot/daily_report.py` | Общий production service формирования ежедневного отчёта | created | Единый Weather/NOAA fetch, climate cache hit/miss, local `ClimateNormalDay`, all-eight assessment, climate-only fallback, DailySummary и concise risk block для manual `/today` и scheduled dispatch |
| `src/weather_alert_bot/daily_dispatch.py` | Pure due decision и однократный scheduled dispatch tick | created | Exact local `HH:MM`, ISO weekday, onboarding/daily-sending gates, local-date idempotency, report/send/state ordering; permanent loop не содержит |
| `src/weather_alert_bot/runtime_state.py` | Отдельное operational runtime SQLite | created | `daily_delivery_state` с per-chat successful local date и UTC timestamp; atomic upsert, validation, corruption errors, без event tables |
...
| `tests/test_today_handler.py` | One-shot `/today`, owner validation, data clients, climate cache и risk composition | updated | Проверяет очистку старых updates, обе формы команды, private/group/чужие chats, onboarding, single-fetch reuse, cache hit/miss, fallback, `daily_sending_enabled = 0`, safe errors, Telegram errors, fixed aware time и read-only settings через fake/mock и temporary SQLite |
| `tests/test_daily_summary.py` | Daily summary builder, formatter и concise preference-filtered risk block | updated | Проверяет все 8 display names, detector order, explicit `ice -> warning_icing_enabled`, disabled categories, climate fallback note, urgent/daily-sending independence и deterministic composition |
...
| `tests/test_today_cli.py` | CLI-режим `--wait-for-today` | updated | Проверяет обязательный token, read-only settings storage, configured separate climate cache path, Historical dependency, current aware datetime, safe storage error и mutual exclusion со всеми actions |
| `tests/test_daily_report.py` | Shared production daily report service | created | Проверяет single-fetch reuse, одинаковые объекты для summary/detector и climate-only fallback через fake/mock |
| `tests/test_runtime_state.py` | Persistent scheduled-delivery operational state | created | Проверяет создание отдельной SQLite, missing state, roundtrip UTC, malformed values, naive datetime, chat isolation и overwrite |
| `tests/test_daily_dispatch.py` | Pure due decision и one-shot dispatch semantics | created | Проверяет local timezone/date/weekday/DST, exact minute, gates, no-request paths, send ordering, idempotency и failure semantics |
| `tests/test_daily_dispatch_cli.py` | CLI-режим `--run-daily-dispatch-once` | created | Проверяет token-safe wiring, read-only settings, runtime path, deterministic output, aware current time и mutual exclusion |

## Актуальная запись: production `/today` all-eight risk assessment закрыт

Baseline stage: `cff6720f8464975cb2ed6f709ccce316bda1662a`. Production `/today` использует saved `UserSettings`, Weather Forecast, NOAA Kp, отдельный writable climate SQLite cache, существующий detector и DailySummary. Weather/NOAA parsed objects переиспользуются; settings SQLite остаётся read-only, а climate SQLite — отдельным writable cache.

Cache hit не вызывает Historical API; cache miss выполняет Historical `1991–2020` → `ClimateNormals` → atomic save → current local `ClimateNormalDay` → all-eight detector. Категории идут в detector order: `magnetic_storm`, `heat`, `cold`, `ice`, `heavy_rain`, `thunderstorm`, `strong_wind`, `storm`. User-facing risk block фильтруется по warning settings с явным mapping `ice -> warning_icing_enabled`; `urgent_warnings_enabled` и `daily_sending_enabled` manual `/today` не блокируют.

Обычная Telegram-сводка использует concise risk block без diagnostic `RiskSignal.reason`; длинные причины остаются в `--preview-current-risks`. При climate-only failure сохраняется six-category compatibility mode с сообщением о временно неоценённых включённых heat/cold; `RiskAssessmentError` не маскируется под этот fallback.

Controlled real `--wait-for-today` cache-hit validation выполнен 14.08.2026 вручную. Guard блокировал только `archive-api.open-meteo.com`; terminal proof: `Ожидание новой команды /today...`, `Команда /today получена.`, `Сводка /today отправлена.`, `HISTORICAL_DNS_ATTEMPTS=0`. Реальный Telegram message содержал `Риски сегодня: значимых по включённым категориям не выявлено.` и weather/Kp summary для Москвы 14 августа 2026. All-eight path, cache hit, owner-only handler, Telegram send и штатное завершение подтверждены; positive real hazard case не утверждается.

До и после запуска production settings SQLite имела SHA-256 `dd249d550da41f27c6d2081b8012eff7593964fb355425c4539e9a23fd077424`, byte-for-byte unchanged. Climate cache был создан до теста. После implementation полный набор: **418 tests OK**; `compileall` и `git diff --check` успешны. Следующий архитектурный этап выбирается техническим лидером отдельно; scheduled sender, scheduler, event lifecycle/persistence, five-day warnings, dedup, notification workflows, NOAA alerts, custom thresholds, permanent polling и systemd не реализованы. `technical_spec.md` unchanged; `next_steps.md` отсутствует; commit/push не выполнялись.
...
## Финальный stop-point: `/today` all-eight risk assessment закрыт — 14.08.2026

`src/weather_alert_bot/today_handler.py` и `src/weather_alert_bot/daily_summary.py` теперь production-интегрируют saved `UserSettings`, Weather Forecast, NOAA Kp forecast, отдельный climate SQLite cache, существующий all-eight detector и concise user-facing risk block. Weather/NOAA parsed objects переиспользуются; settings SQLite read-only, climate SQLite writable и отдельная. Cache hit не вызывает Historical API, cache miss сохраняет рассчитанный snapshot после Historical `1991–2020`.

Risk categories идут в порядке `magnetic_storm`, `heat`, `cold`, `ice`, `heavy_rain`, `thunderstorm`, `strong_wind`, `storm`; mapping `ice -> warning_icing_enabled` зафиксирован без переименования storage category. `urgent_warnings_enabled` и `daily_sending_enabled` manual `/today` не блокируют. Diagnostic reasons остаются в `--preview-current-risks`; climate-only failure сохраняет six-category fallback и сообщает о временно неоценённых включённых heat/cold, а `RiskAssessmentError` не маскируется.

Controlled real `--wait-for-today` cache-hit test выполнен 14.08.2026: `HISTORICAL_DNS_ATTEMPTS=0`, реальное сообщение содержало `Риски сегодня: значимых по включённым категориям не выявлено.`. Подтверждены private owner-only handler, Weather + NOAA summary, climate cache hit, all-eight path, Telegram send и штатное завершение. Positive real hazard case не утверждается. Settings SQLite до/после byte-for-byte unchanged, SHA-256 `dd249d550da41f27c6d2081b8012eff7593964fb355425c4539e9a23fd077424`; full suite **418 tests OK**, compileall и `git diff --check` успешны.

Следующий архитектурный этап отдельно выбирает технический лидер. Scheduled sender, scheduler, event persistence/lifecycle, five-day warnings, dedup, notification workflows, NOAA watches/warnings/alerts, custom thresholds, permanent polling и systemd не реализованы. `technical_spec.md` unchanged; `next_steps.md` отсутствует; commit/push не выполнялись.

## Финальный stop-point: scheduled daily dispatch core реализован

Baseline stage: `4891eb94ed19c9b596fa0aadf55ba372a1b8cb00`. `daily_report.py` — общий production service для manual `/today` и scheduled tick: один Weather/NOAA fetch, climate cache hit/miss, local `ClimateNormalDay`, all-eight assessment, climate-only fallback, DailySummary и concise risk block.

`daily_dispatch.py` выполняет exact local-minute due decision по `ZoneInfo(settings.timezone)`, ISO weekday, onboarding/daily-sending gates и local-date idempotency. `runtime_state.py` использует отдельный `runtime.sqlite3` с `daily_delivery_state`; state пишется только после успешного Telegram send, climate cache для dedup не используется. `--run-daily-dispatch-once` не содержит polling, sleep или loop; not-due/already-sent не вызывают внешние data sources и Telegram send.

После implementation: **454 tests OK**, compileall и `git diff --check` успешно. Controlled real scheduled send ещё НЕ выполнялся. Следующий stop-point — controlled real one-shot dispatch validation. `technical_spec.md` не изменён; `next_steps.md` отсутствует; commit/push не выполнялись.

## Финальный stop-point: scheduled daily dispatch core validated 14.08.2026

Stage baseline: `4891eb94ed19c9b596fa0aadf55ba372a1b8cb00`. `daily_report.py` — единый reusable production service для manual `/today` и scheduled dispatch: Weather, NOAA Kp, climate cache hit/miss, `ClimateNormalDay`, all-eight assessment, climate-only six-category fallback, `DailySummary`, concise risk section и final Telegram text. Manual `/today` использует этот service и не зависит от runtime delivery state; отдельной копии report logic в scheduled sender нет.

`daily_dispatch.py` остаётся one-shot tick: due по сохранённой local timezone, ISO weekday, exact local `HH:MM`, onboarding/daily-sending gates и local-date dedup; catch-up semantics нет. `runtime_state.py` — отдельная operational DB с `daily_delivery_state`, Telegram chat-id identity, `last_successful_local_date` и UTC send timestamp; state mark только после успешного Telegram send. `--run-daily-dispatch-once` не содержит polling, sleep, permanent scheduler или daemon.

Controlled real send подтверждён через temporary settings/runtime SQLite и production climate cache: target `2026-08-14 15:08 Europe/Moscow`, `HISTORICAL_DNS_ATTEMPTS=0`, реальный Weather/NOAA/Telegram path и сообщение с погодой, Kp и concise risk block. Runtime DB получила ровно одну state row для test owner. Второй tick в `15:12` той же local date с DNS guard завершился `Ежедневная сводка за текущий день уже отправлена.` и `DNS_ATTEMPTS=0`, то есть duplicate был подавлен до любых data-source/network requests. Production settings SHA-256 до/после совпал: `dd249d550da41f27c6d2081b8012eff7593964fb355425c4539e9a23fd077424`.

**454 tests OK**, compileall и `git diff --check` успешно. `technical_spec.md` unchanged; `next_steps.md` отсутствует; temporary DB/secrets не добавлены. Permanent scheduler, daemon, systemd, event lifecycle/notifications, urgent/NOAA alert delivery, normalization/custom thresholds, `/settings` и `/help` остаются вне реализации; следующий этап отдельно выбирает технический лидер.

## Permanent scheduler stage — 14.08.2026

- `src/weather_alert_bot/scheduler.py` — отдельный sequential permanent scheduler: explicit aware UTC tick time, settings reread, вызов одного existing `run_daily_dispatch_once`, pure next-minute-boundary helpers, interruptible wait через `threading.Event`, safe structured failed outcome и stable success/error diagnostics. Due/report/network/runtime semantics здесь не дублируются.
- `src/weather_alert_bot/app.py` — добавлен mutually exclusive foreground CLI `--run-scheduler`, production dependency wiring из configured settings/climate/runtime paths, Telegram token requirement и SIGINT/SIGTERM stop handlers. `--run-daily-dispatch-once` и manual `/today` не изменены.
- `tests/test_scheduler.py` — cadence, UTC boundary, drift/no-catch-up, settings reread, idle network invariant, tick failure isolation и graceful stop coverage.
- `tests/test_scheduler_cli.py` — token requirement, mutual exclusion, stable start/stop diagnostics, signal wiring и configured read-only/writable dependency paths.

Permanent scheduler сейчас регистрирует и выполняет только daily-dispatch job. Five-day/hourly/active-hazard/NOAA alert jobs не добавлялись; Telegram update polling, threads/processes, systemd и cron отсутствуют. Полный набор после stage: 470 tests OK. Controlled real scheduler validation ещё не выполнена; текущий stop-point — foreground запуск с внешним timeout/signal.

## Permanent foreground scheduler stage — закрыт controlled-real validation 14.08.2026

`src/weather_alert_bot/scheduler.py` — permanent sequential foreground loop с немедленным initial tick и последующими UTC minute-boundary ticks. Используются новые settings на каждом tick, existing `run_daily_dispatch_once`, interruptible stop event/wait и safe failure isolation. Fixed `sleep(60)`, catch-up/replay, parallel jobs, thread pool и asyncio не добавлялись; due/local-time/report/climate/risk/runtime-dedup semantics не дублируются.

`--run-scheduler` остаётся обычным foreground process и поддерживает KeyboardInterrupt/SIGTERM. Idle `not due`/`already sent` ticks не вызывают data providers или Telegram и не печатают success diagnostics; recoverable tick failure переносится на следующую будущую minute boundary. Settings SQLite read-only, runtime state — отдельная writable SQLite.

Controlled real run использовал `/tmp/weather-alert-scheduler-test/settings.sqlite3`, `/tmp/weather-alert-scheduler-test/runtime.sqlite3`, timezone owner `Europe/Moscow`, temporary enabled schedule и existing production climate cache. Ошибка первой wrapper attempt (`timeout` получил `PYTHONPATH=src` как executable, exit `127`) не была scheduler failure; после исправления wrapper production `--run-scheduler` реально дошёл до due minute и отправил daily report в Telegram в `14.08.2026 16:17 Europe/Moscow`. Manual Telegram commands не использовались.
