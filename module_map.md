...
| `src/weather_alert_bot/storage.py` | SQLite-хранилище пользовательских настроек | created | Только `sqlite3`; создаёт схему при явной инициализации, upsert по `telegram_chat_id`, без вывода данных |
...
| `src/weather_alert_bot/confirmed_city_handler.py` | Однократный Telegram-сценарий подтверждения найденного города | updated | После «Да» сохраняет подтверждённый `GeocodingLocation` через переданное хранилище и отправляет итоговый текст только после успешной записи |
...
- Автоматические тесты используют временные SQLite-базы; реальный Telegram-тест новой логики сохранения в этом run не выполнялся.
- Следующий безопасный шаг после независимой проверки commit — контролируемый реальный тест сохранения. Настройка времени рассылки, дней недели, прогноз и другие этапы не начинались.
...
- Автоматические тесты новой логики выполнены. Реальный Telegram-тест сохранения в этом run НЕ выполнен.
- Следующий безопасный шаг — контролируемый реальный Telegram-тест сохранения. Время рассылки, дни недели, прогноз погоды и следующие функциональные этапы не начинались.
