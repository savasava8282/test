...
- Настоящий запрос к Open-Meteo в этой задаче не выполнялся; все сетевые сценарии геокодирования проверяются через mock.
