...
| `src/weather_alert_bot/app.py` | CLI, обычный запуск, `--check-telegram`, `--wait-for-start`, `--wait-for-city`, `--wait-for-geocoded-city`, `--wait-for-confirmed-city`, `--wait-for-daily-time` и `--geocode-city` | updated | Все режимы взаимоисключающие; обычный запуск и прежние режимы сохранены |
...
| `src/weather_alert_bot/storage.py` | SQLite-хранилище пользовательских настроек | updated | Только `sqlite3`; безопасно мигрирует `daily_send_time` с default `07:00`, сохраняет upsert города без сброса времени и отдельно обновляет время по существующему `telegram_chat_id` |
...
| `src/weather_alert_bot/daily_time_handler.py` | Однократная настройка локального времени ежедневной отправки | created | После новой приватной `/start` проверяет сохранённый город, принимает только `HH:MM`, сохраняет время без UTC-конвертации и безопасно завершается; подтверждён автоматическими тестами, реальный Telegram-тест не выполнялся |
...
| `tests/test_storage.py` | Схема, миграция, upsert и изоляция настроек разных Telegram-чатов | updated | Проверяет `daily_send_time`, default `07:00`, безопасную миграцию и сохранение изменённого времени; использует только временные SQLite-базы |
...
- Новый режим подтверждён автоматическими тестами. Реальный Telegram-тест нового режима в этом run не выполнялся.
