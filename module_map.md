...
| `src/weather_alert_bot/confirmed_city_handler.py` | Однократный Telegram-сценарий подтверждения найденного города | created | Реально подтверждена цепочка `/start` → «Москва» → геокодирование → «Да» → финальное сообщение; город, координаты и часовой пояс не сохраняет |
